...
public class MultiplayerLobby extends JPanel {
    ...
    private void buildHostPanel() {
        ...
        copyBtn.setFont(new Font("Tahoma",Font.BOLD,13)); copyBtn.setForeground(WHITE);
        copyBtn.setOpaque(false); copyBtn.setContentAreaFilled(false); copyBtn.setBorderPainted(false); copyBtn.setFocusPainted(false);
        copyBtn.setBorder(new EmptyBorder(6,12,6,12)); copyBtn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        ...
        JPanel ipCard = new JPanel(){
            @Override protected void paintComponent(Graphics g){
                ...
                g2.setColor(new Color(170,215,170)); g2.setStroke(new BasicStroke(1.5f));
                g2.draw(new RoundRectangle2D.Float(0,0,getWidth()-1,getHeight()-1,20,20)); g2.dispose();
            }
        };
        ipCard.setLayout(new BorderLayout(10,0)); ipCard.setOpaque(false);
    ...
    private void buildWaitingPanel() {
        ...
        JLabel status=lbl("เชื่อมต่อสำเร็จ! รอ Host เริ่มเกม...",F18B,GREEN);
        ...
        scrollW.setMaximumSize(new Dimension(440,200)); scrollW.setAlignmentX(CENTER_ALIGNMENT);
        ...
        box.add(title); box.add(gap(10)); box.add(status); box.add(gap(6));
    ...
    public void showLeaderboard(Map<String,Integer> scores) {
        ...
        String[] ranks={"1.","2.","3."};
        Color[] cols={new Color(190,140,0),new Color(120,120,120),new Color(150,90,40)};
        for(int i=0;i<sorted.size();i++){
            ...
            JLabel sl=new JLabel(sorted.get(i).getValue()+" คะแนน"); sl.setFont(F24B); sl.setForeground(i<cols.length?cols[i]:cols[cols.length-1]);
